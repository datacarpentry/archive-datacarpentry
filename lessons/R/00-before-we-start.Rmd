...
The console window (in R studio, the bottom left panel) is the place where R is waiting for you to tell it what to do, and where it will show the results of a command. If R is ready to accept commands, it shows a `>` prompt. In most cases, you send commands from the script editor (top left window) to the console window. This way, you have complete record of what you did, and you can do it again later on if needed. A shortcut for this is `Ctrl-Enter`. This will send the current script editor line, or the currently selected text in the  script editor, to the console. R will then try to execute it, and when ready, come back with a new `>`-prompt.
However, if R is still waiting for you to enter more data because it isn't complete yet, the console will show a `+` prompt. It means that you haven't finished entering a complete command. This is because you have not 'closed' a parenthesis or quotation. If this happens, fix it by entering the approriate closing character. If it is difficult to spot the right 'closing character', try each of the following single characters in turn until the '>'- prompt returns (often with an error).
`"` ,   `)`,  `}`,   `]`   or `'`.
