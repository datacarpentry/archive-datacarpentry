...
window and pres `Esc`; this should help you out of trouble. When using plain R
(i.e. outside Rstudio), you have to fix it by entering the approriate closing
character. If it is difficult to spot the right 'closing character', try each of
the following single characters in turn until the `>`- prompt returns: `"` ,
`)`, `}`, `]` or `'`.
