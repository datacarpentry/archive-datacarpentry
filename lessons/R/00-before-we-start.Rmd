...
becoming available when using a `library` (more on that later). A function usually gets one or more inputs called *arguments*. Functions often (but not always) return a *value*. 
...
Here, the value of `a` is given to the `sqrt`-function, the `sqrt`-function calculates the square root, and returns 
...
way the function operates, such as whether to ignore 'bad values', such an argument is sometimes called an *option*. 
...
fall back on using the *default*. This is a standard value that the author of the function specified as being 'good enough in standard cases'. An example would be what symbol to use in a plot. However, if you want something specific, simply specify the argument yourself with a value of your choice.

To see these default values, you can either type `args(read.csv)` or look at the help for this function (e.g., `?read.csv`).
